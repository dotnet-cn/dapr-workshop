...
3. Open the file `FineCollectionService/Proxies/VehicleRegistrationService.cs` in VS Code.
...
5. Add a reference to the Dapr ASP.NET Core integration library:
...
2. Add a using statement in this file to make sure you can use the Dapr client:

    ```csharp
    using Dapr.Client;
    ```

3. The `ConfigureServices` method, contains these two lines of code which register the .NET `HttpClient` and the `VehicleRegistrationService`  proxy (which uses the `HttpClient`) with dependency injection:
...
4. Replace these two lines with with the following lines:
...
5. Open the file `FineCollectionService/Proxies/VehicleRegistrationService.cs` in VS Code.

6. Because the `HttpClient` passed into this class has already been created for a certain `app-id`, you can omit the host information from the request URL. Change the URL that is used in the `GetVehicleInfo` to `/vehicleinfo/{license-number}`. The method should now look like this:
